...
| Install | Local checkout | Project-scoped locked dependency installation completes | PASS | Pinned generator install plus exact compatibility dependencies; fresh-clone `npm ci` exited 0 |
| Typecheck | Local checkout | `npm run typecheck` exits 0 | PASS | Final aggregate gate and fresh-clone aggregate gate both passed |
| Lint | Local checkout | `npm run lint` exits 0 | PASS | Final aggregate gate and fresh-clone aggregate gate both passed |
| Production build | Local checkout | `npm run build` exits 0 using generated Vinext build | PASS | Five-stage Vinext/Vite build passed in the source checkout and fresh clone |
...
| Clean-checkout verification | Fresh no-hardlinks clone of `382a4d1ef02cf437bd765602c55ac0fc0d43146c` | `npm ci`, `npm run verify:package0`, minimal Worker request, clean Git state | PASS | All commands exited 0; 28,522-byte response contained all four expected markers; clone remained clean; `.artifacts/test/package0-clean-checkout.json` |
