...
The four screenshots below were generated by the current-source Playwright
journey with synthetic data and reviewed for private data. They are committed
with the R11 candidate so any later saved/deployed build and the repository
presentation can be checked against one visual implementation.
