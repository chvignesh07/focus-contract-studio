...
- 32 new-workspace bootstraps/minute per rotating client digest only when a strictly validated `CF-Connecting-IP` is present; the application does not read `request.cf`; `X-Forwarded-For`, `X-Real-IP`, and other caller-controlled headers cannot select or change the bucket; absence or malformed input returns `503` before any workspace/product write;
