...
| `E-010` | Accessibility automation | `.artifacts/accessibility/axe.json` | No serious/critical automated violations on every key state. | `C` + browser/version | `PASS` frozen/local automation plus Package 8 built-Worker 320/375/640-CSS-px-at-DPR-2 keyboard/reflow/Axe checks; actual browser UI 200% zoom and manual evidence remain `NOT_RUN` |
...
| `E-018` | Review 1 | `docs/evidence/ADVERSARIAL_REVIEW_1.md` | Local authority/security/retrieval/UX audit; every finding disposition and retest. | Reviewed commit(s), final `C` candidate | `LOCAL PASS` |
